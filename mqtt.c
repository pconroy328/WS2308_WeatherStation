...
int     sendNumericAlarm (WS2308System_t *aSystem, char *alarmMessage, double temp)
{
    ...
    if (!aSystem->MQTTUseJSON) {
        ...
        //
        // Send the data in a JSON packet: 'Topic { "datetime":"<dt in ISO8601>", "alarmMsg":"<msg>", "value":<value> }'
        
        length = snprintf( payload, sizeof payload,
                    "%s { \"datetime\":\"%s\" , \"alarmMsg\":\"%s\" , \"value\":%0.1f }",
                    combinedTopic,
                    getCurrentDateTime(),
                    alarmMessage,
                    temp );
...
int     sendWindAlarm (WS2308System_t *aSystem, char *alarmMessage, double windspeed, double windHeading)
{
    ...
    if (!aSystem->MQTTUseJSON) {
        ...
        length = snprintf( payload, sizeof payload,
                "%s { \"datetime\":\"%s\" , \"alarmMsg\":\"%s\" , \"speed\":%0.1f , \"heading\":%03.1f }",
                combinedTopic,
                getCurrentDateTime(),
                alarmMessage,
                windspeed,
...
int     sendRainAlarm (WS2308System_t *aSystem, char *alarmMessage, double lastHour, double lastDay, double total)
{
    ...
    if (!aSystem->MQTTUseJSON) {
        ...
        length = snprintf( payload, sizeof payload,
                "%s { \"datetime\":\"%s\" , \"alarmMsg\":\"%s\" , \"lastHour\":%0.2f , \"lastDay\":%0.2f , \"total\":%0.2f }",
                combinedTopic,
                getCurrentDateTime(),
                alarmMessage,
                lastHour,
...
int    MQTT_createWeatherStatus (WS2308System_t *aSystem, weatherDatum_t *datum)
{
    ...
    if (!aSystem->MQTTUseJSON) {
        ...
        length = snprintf( buffer, sizeof buffer,
                "%s { \"datetime\":\"%s\" ,\"tendency\":\"%s\" ,\"forecast\":\"%s\" ,\
