...
//static  int     OldLogger_Log (char *format, char *level, ...);
...
// ----------------------------------------------------------------------------
void    OldLogger_Initialize (char *fileName, int debugLevel)
{
    ...
    //fprintf( stderr, "[%s] OldLogger_Initialize() called - Opening up the file.\n", getCurrentDateTime() );
    if (fileName != (char *) 0 ) {
        fp = fopen( fileName, "a" );
        if (fp != (FILE *) 0) {
            logFileOpen = TRUE;
    ...
    if (logFileOpen)
        //fprintf( stderr, "[%s] OldLogger_Initialize() called - Log File is open.\n", getCurrentDateTime() );
        ;
}

// ----------------------------------------------------------------------------
void    OldLogger_UpdateDebugLevel (int newLevel)
{
    debugValue = newLevel;
}
...
// ----------------------------------------------------------------------------
void    OldLogger_LogDebug (char *format, ... )
{
...
// ----------------------------------------------------------------------------
void    OldLogger_LogWarning (char *format, ... )
{
...
// ----------------------------------------------------------------------------
void    OldLogger_LogError (char *format, ... )
{
...
// ----------------------------------------------------------------------------
void    OldLogger_LogFatal (char *format, ... )
{
...
// ----------------------------------------------------------------------------
void    OldLogger_LogInfo (char *format, ... )
{
...
/*
// ----------------------------------------------------------------------------
static  int    OldLogger_Log (char *level, char *format, ...)
{
    if (!logFileOpen)
        return 0;
